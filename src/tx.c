 ...
 * Copyright 2013-2014Intel Mobile Communications GmbH
					...
					struct sk_buff *skb)
{
	...
	while (*frag_tail) {
		nfrags += 1 + skb_shinfo(skb_tmp)->nr_frags;
		frag_tail = &(skb_tmp)->next;
		n++;
	}
	 ...
	 * andfrag_tail should be &skb_shinfo(head)->frag_list.
