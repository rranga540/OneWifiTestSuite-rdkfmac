...
static bool mac80211_hwsim_addr_match(struct mac80211_rdkfmac_data *data, const u8 *addr);
					...
					struct net_device *dev)
{
	...
	if (ntohs(eth_hdr->h_proto) != 34958) {
		...
		printk(KERN_INFO "hwsim_mon_xmit: proto=0x%04x freq=%u fc=0x%04x type=%u stype=%u addr1=%pM addr2=%pM multicast=%d\n",
		...
		printk(KERN_INFO "hwsim_mon_xmit: raw EAPOL (0x888e), skipping addr filter\n");
	}
	...
	list_for_each_entry(nic, &hwsim_radios, list) {
		...
		if(nic->idle || !nic->started || !nic->channel)
			continue;
		/* unicast: only deliver to the radio whose VIF owns addr1 */
		if (hdr80211 && !is_multicast_ether_addr(hdr80211->addr1) &&
		    !mac80211_hwsim_addr_match(nic, hdr80211->addr1)) {
			printk(KERN_INFO "hwsim_mon_xmit: skip radio %pM (addr1=%pM no match)\n",
		...
		printk(KERN_INFO "hwsim_mon_xmit: inject to radio %pM\n",
