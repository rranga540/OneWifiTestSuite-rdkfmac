...
static void ieee80211_beacon_connection_loss_work(struct work_struct *work)
{
	...
	if (ifmgd->connection_loss) {
		...
		// This will flood probes as we dont have beacon
				   ...
				   struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (auth_alg != ifmgd->auth_data->algorithm ||
	    ...
	    (auth_alg == WLAN_AUTH_SAE &&
	     (auth_transaction < ifmgd->auth_data->expected_transaction ||
	      auth_transaction > 2))) {
		printk("%pM unexpected authentication state: alg %d (expected %d) transact %d (expected %d)\n",
	...
	if (status_code != WLAN_STATUS_SUCCESS) {
		...
		printk("%pM denied authentication (status %d)\n",
			   mgmt->sa, status_code);
...
static int ieee80211_auth(struct ieee80211_sub_if_data *sdata)
{
	...
	if (auth_data->tries > IEEE80211_AUTH_MAX_TRIES) {
		printk("authentication with %pM timed out\n",
			   auth_data->bss->bssid);
	...
	printk("send auth to %pM (try %d/%d)\n",
		       ...
		       struct cfg80211_auth_request *req)
{
	...
	if (ifmgd->associated) {
		...
		printk("disconnect from AP %pM for new auth to %pM\n",
